...
floyd, bellman_ford: DP
dijkstra: 贪心

d[k,i,j]: 所有从i出发，最终走到j，且中间只经过节点编号不超过k的所有路径
> 注意理解是节点标号. 不包含i,j
路径长度最小值

集合划分
1. 中间经过k: 经过i->k. d[k-1,i, k]+d[k-1,k,j]
2. 中间不经过k: 说明经过的是d[k-1,i,j]

### 4.3 最小生成树
最小生成树中所有的边都是无向边

在当前的连通情况下。选择未连接点中到该连通块最小的点的边。
> 假设不是选取的最小的边。否则可以构造更小的生成树
> 根据环来做

kruskal: 基于并查集，选择两个顶点不在一个连通块的情况

核心证明：如果当前不选，可以在最优解（一棵树）中加上。然后形成一个环，形成一个环一定存在小于该边权的边，因此该边一定可以存在最优解中
...
负环是什么？
在求最短路时，如果原图中存在负环，那么dist[t]就会一直得到更新。
...
推荐第二种，因为在某种情况下，1为n^2, 2为n

新图上超级源点

1. spfa存在负权边，会被更新，不是-inf.
2. 为什么等价于将所有点入队

> 理解负环一定是在最短路上运用的！ 但是这里并不是说最短路占用更多的节点。而是利用了最短路，每次更新，实际上是根据一条新边来更新一个新点。 而负环其实就是看一个环的点数是否大于n了。  有负环，在这个算法中边一直都能被更新

\sum_{fi} / \sum_{ti} 最大 =>  01分数规划
求一个环上的。
...
1. 向上标记法： 对点a向上走到根节点的路径进行标记，同时对b点做，当向上走的祖先相同时，即得到O(n)
2. 倍增fa[i,j]: 表示从i开始向上走2^j能到达的节点。 0 <= j <= logn; depth[i]表示深度，层数，即到根节点的距离+1
    - j = 0, f[i, j]=i的父节点
    - j > 0,  f[i, j] = f(f[i, j-1],j-1)
    > x,y  基于二进制拼凑的思想. t>=2^k,  t包含k位
    > 哨兵： 如果从i开始跳2^j步会跳过根节点，那么fa[i,j]=0, depth[0]=0
    > 步骤:  预处理O(nlogn)?  查询O(mlogn)
    > 1. 先把x,y跳到同一层。 depth[x]-depth[y].  depth(f[x,k]) >= depth(y) logn。 从大到小
    > 2. 让两个点一直往上跳，跳到同时是公共祖先的下一层为止。 跳到不等时 logn
3. Tarjan: 离线求LCA 查询O(n+m) 
    > DFS，合并两个集合可以用并查集来做。 遍历，合并，查询一次。 O(1), 合并和查询的时间都是O(1)
    - 2. 已经搜索过的点
    - 1. 正在搜索的点
    - 0. 还未搜索的点
    > 根据并查集来做！
...
### 4.7 有向图的强连通分量
一般来说，对于一般的图很难做。所以我们常见的想法就是先将一个图缩点为有向芜湖安图DAG, 拓扑图，然后用关键路径来做
...
如何找所有边的双连通分量？
- 将所有桥删掉，剩下的连通分支都是
- 使用栈来找 dfn[x]==low[x]:  x所包含的子树就是双连通分量

冗余路径：如果两个路径没有公共边，等价于边的双连通分量
最少加几条边可以构成边的双连通分量
缩点后其实就是一棵树，所有度数为1的点都至少要加一条边。
ceil(cnt/2) = (cnt+1)/2
证明：
充分性：反证法
必要性：
...
low[y] >= dfn[x]
1. 如果x不是根节点，那么x是割点
2. 如果x是根节点，那么至少有两个子节点yi, 满足low[yi] >= dfn[x]
电力：统计连通块个数 cnt；依次枚举从哪个块中删，再枚举删除哪个点？s.
依次枚举每个割点然后求全局最大值
...
起点：(0,0)
终点: f[0,0], f[0,1], ..., f[0,m]

状态转移方程
f[i,j] = min(f[i,j], f[i-1,j] + w)
其实f[i,j]就可以看作是一个点。然后加的值表示权

然后，好像对于方案数为什么是相加，豁然开朗
